...
public final class OniGamePlugin extends JavaPlugin implements Listener, CommandExecutor, TabCompleter {
    ...
    @Override public boolean onCommand(CommandSender sender,Command command,String label,String[] a){
        ...
        if(a[0].equalsIgnoreCase("automap")&&sender instanceof Player p){
            if(a.length>=2&&a[1].equalsIgnoreCase("maxy")){
                if(a.length==2){msg(p,"&e自動配置スポーン上限: &fY="+getConfig().getInt("arena-setup.spawn-max-y",90));return true;}
                try{int maxY=Integer.parseInt(a[2]);int worldMin=p.getWorld().getMinHeight()+2,worldMax=p.getWorld().getMaxHeight()-3;if(maxY<worldMin||maxY>worldMax){msg(p,"&cmax-y は "+worldMin+"～"+worldMax+" の範囲で指定してください。");return true;}getConfig().set("arena-setup.spawn-max-y",maxY);saveConfig();msg(p,"&a自動配置の鬼・ぷれいやースポーン上限を &eY="+maxY+" &aに設定しました。");return true;}catch(NumberFormatException ex){msg(p,"&c使用法: /og automap maxy <Y>");return true;}
            }
    ...
    private Location randomSpawnSurface(Location center,Random random,int half,List<Location> occupied,double separation){World world=center.getWorld();int maxY=Math.min(getConfig().getInt("arena-setup.spawn-max-y",90),world.getMaxHeight()-3);for(int attempt=0;attempt<180;attempt++){int x=center.getBlockX()+random.nextInt(half*2+1)-half,z=center.getBlockZ()+random.nextInt(half*2+1)-half;for(int y=maxY-1;y>=world.getMinHeight()+1;y--){Block ground=world.getBlockAt(x,y,z);Location location=new Location(world,x+.5,y+1,z+.5,center.getYaw(),center.getPitch());if(!ground.getType().isSolid()||ground.isLiquid()||ground.getType().name().contains("LEAVES")||!location.getBlock().isPassable()||!location.clone().add(0,1,0).getBlock().isPassable())continue;boolean clear=true;for(Location other:occupied)if(other.getWorld().equals(world)&&other.distanceSquared(location)<separation*separation){clear=false;break;}if(clear)return location;break;}}Location fallback=center.clone();fallback.setY(Math.min(maxY,Math.max(world.getMinHeight()+2,center.getY())));return fallback;}
